...
@Component
@Slf4j
public class BiWeeklyPayStubJob extends BaseSchedulerJob {
    ...
    protected JobResult executeJob(JobExecutionContext context) throws Exception {
        ...
        List<WorkerPayroll> payrolls = payrollRepository
                ...
                .filter(p -> {
                    LocalDate firstBiweeklyDate = p.getCompany().getFirstBiweeklyDate();
                    if (firstBiweeklyDate == null) {
                        return true;
                    }
                    long weeks = ChronoUnit.WEEKS.between(firstBiweeklyDate, today);
                    return weeks % 2 == 0;
                })
                .toList();

        log.info("Found {} BI-WEEKLY payrolls to process", payrolls.size());
