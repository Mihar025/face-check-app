...
public class CompanyControllerTest {
    ...
    @Test
    @WithMockUser(roles = "ADMIN")
    void updateCompanyName_thenReturnOk_andCallService() throws Exception {
        String newName = "Acme Corp";

        // Выполняем PUT, контент – просто текст, не JSON
        mockMvc.perform(put("/company/update-name")
                        .contentType(MediaType.TEXT_PLAIN)
                        .content(newName))
                .andExpect(status().isOk())
                .andExpect(content().string("")); // тело пустое

        // Проверяем, что сервис получил ровно ту же строку
        then(companyService).should(times(1))
                .updateCompanyName(eq(newName), isNull());
    }

    @Test
    @WithMockUser(roles = "ADMIN")
    void updateCompanyAddress_thenReturnOk_andCallService() throws Exception {
        String newAddress = "Acme Corp";

        mockMvc.perform(put("/company/update-address")
                        .contentType(MediaType.TEXT_PLAIN)
                        .content(newAddress))
                .andExpect(status().isOk())
                .andExpect(content().string("")); // тело пустое
        ...
        // Проверяем, что сервис получил ровно ту же строку
        then(companyService).should(times(1))
                .updateCompanyAddress(eq(newAddress), isNull());
    }

    @Test
    @WithMockUser(roles = "ADMIN")
    void updateCompanyPhone_thenReturnOk_andCallService() throws Exception {
        String phoneNumber = "3456789";

        mockMvc.perform(put("/company/update-phone")
                        .contentType(MediaType.TEXT_PLAIN)
                        .content(phoneNumber))
                .andExpect(status().isOk())
                .andExpect(content().string("")); // тело пустое
        ...
        // Проверяем, что сервис получил ровно ту же строку
        then(companyService).should(times(1))
                .updateCompanyPhoneNumber(eq(phoneNumber), isNull());
    }

    @Test
    @WithMockUser(roles = "ADMIN")
    void updateCompanyEmail_thenReturnOk_andCallService() throws Exception {
        String email = "3456789";

        mockMvc.perform(put("/company/update-email")
                        .contentType(MediaType.TEXT_PLAIN)
                        .content(email))
                .andExpect(status().isOk())
                .andExpect(content().string("")); // тело пустое
        ...
        // Проверяем, что сервис получил ровно ту же строку
        then(companyService).should(times(1))
                .updateCompanyEmail(eq(email), isNull());
    }
